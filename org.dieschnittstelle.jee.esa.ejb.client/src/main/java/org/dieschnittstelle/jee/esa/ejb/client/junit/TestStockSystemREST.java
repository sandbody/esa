...
import org.dieschnittstelle.jee.esa.ejb.client.ejbclients.StockSystemClientREST;
...
public class TestStockSystemREST {
	
	private ProductCRUDClient productCRUDClient;
	private StockSystemClientREST stockSystemClient;
	...
	public void prepareContext() throws Exception {
		...
		stockSystemClient = new StockSystemClientREST();
	...
	public void stockSystemWorks() {
		// add to stock
		stockSystemClient.addToStock(PRODUCT_1.getId(), TOUCHPOINT_1.getErpPointOfSaleId(), 100);
		stockSystemClient.addToStock(PRODUCT_2.getId(), TOUCHPOINT_1.getErpPointOfSaleId(), 50);
		stockSystemClient.addToStock(PRODUCT_1.getId(), TOUCHPOINT_2.getErpPointOfSaleId(), 75);
		
		assertEquals("create/read correct for p1/tp1", 100, stockSystemClient.getUnitsOnStock(PRODUCT_1.getId(), TOUCHPOINT_1.getErpPointOfSaleId()));
		assertEquals("create/read correct for p2/tp1", 50, stockSystemClient.getUnitsOnStock(PRODUCT_2.getId(), TOUCHPOINT_1.getErpPointOfSaleId()));
		assertEquals("create/read correct for p1/tp2", 75, stockSystemClient.getUnitsOnStock(PRODUCT_1.getId(), TOUCHPOINT_2.getErpPointOfSaleId()));

		stockSystemClient.addToStock(PRODUCT_1.getId(), TOUCHPOINT_1.getErpPointOfSaleId(), 5);
		assertEquals("update/read correct for p1/tp1", 105, stockSystemClient.getUnitsOnStock(PRODUCT_1.getId(), TOUCHPOINT_1.getErpPointOfSaleId()));
		
		stockSystemClient.removeFromStock(PRODUCT_1.getId(), TOUCHPOINT_1.getErpPointOfSaleId(), 10);
		assertEquals("remove correct for p1/tp1", 95, stockSystemClient.getUnitsOnStock(PRODUCT_1.getId(), TOUCHPOINT_1.getErpPointOfSaleId()));
		...
		assertEquals("total units on stock correct for p1", 170, stockSystemClient.getTotalUnitsOnStock(PRODUCT_1.getId()));
		
		// check that we get the correct touchpoints for the products
		List<Long> touchpoints_p1 = stockSystemClient.getPointsOfSale(PRODUCT_1.getId());
		List<Long> touchpoints_p2 = stockSystemClient.getPointsOfSale(PRODUCT_2.getId());
